...
public class PdfViewerActivity extends AppCompatActivity {
    ...
    private static final List<String> TAGS=Arrays.asList("Felsefe","Ekonomi","Strateji","Bilim","Tarih","Psikoloji","Diger");
    ...
    class HighlightOverlay extends View {
        ...
        HighlightOverlay(Context ctx) { super(ctx); setBackgroundColor(Color.TRANSPARENT); }

        void loadData(List<long[]> ids,List<float[]> rects) {
            hlIds=new ArrayList<>(ids); hlRects=new ArrayList<>(rects); drawing=false; invalidate();
        }
        void startDraw(float x,float y){startX=x;startY=y;endX=x;endY=y;drawing=true;invalidate();}
        void updateDraw(float x,float y){endX=x;endY=y;invalidate();}
        float[] finishDraw(){
            drawing=false;
            float x1=Math.min(startX,endX),y1=Math.min(startY,endY),x2=Math.max(startX,endX),y2=Math.max(startY,endY);
            invalidate();
            return (Math.abs(x2-x1)<15||Math.abs(y2-y1)<8)?null:new float[]{x1/getWidth(),y1/getHeight(),x2/getWidth(),y2/getHeight()};
        }
        long undoLast(){
            if(hlIds.isEmpty())return -1;
            long id=hlIds.get(hlIds.size()-1)[0];
            hlIds.remove(hlIds.size()-1); hlRects.remove(hlRects.size()-1); invalidate(); return id;
        }
        long removeTouched(float tx,float ty){
            ...
            for(int i=hlRects.size()-1;i>=0;i--){
                ...
                if(nx>=h[0]&&nx<=h[2]&&ny>=h[1]&&ny<=h[3]){
                    long id=hlIds.get(i)[0]; hlIds.remove(i); hlRects.remove(i); invalidate(); return id;
                }
            }
            return -1;
        }
        private int fill(float f){switch((int)f){case 1:return 0x50FF4560;case 2:return 0x504488FF;case 3:return 0x5044CC66;default:return 0x50FFE500;}}
        private int stroke(float f){switch((int)f){case 1:return 0x99FF4560;case 2:return 0x994488FF;case 3:return 0x9944CC66;default:return 0x99DDB800;}}
        @Override protected void onDraw(Canvas canvas){
            super.onDraw(canvas);
            int w=getWidth(),h=getHeight();
            for(float[]hl:hlRects){
                paint.reset(); paint.setStyle(Paint.Style.FILL); paint.setColor(fill(hl[4]));
                canvas.drawRect(hl[0]*w,hl[1]*h,hl[2]*w,hl[3]*h,paint);
                paint.setStyle(Paint.Style.STROKE); paint.setStrokeWidth(1.5f); paint.setColor(stroke(hl[4]));
                canvas.drawRect(hl[0]*w,hl[1]*h,hl[2]*w,hl[3]*h,paint);
            }
            if(drawing){
                float x1=Math.min(startX,endX),y1=Math.min(startY,endY),x2=Math.max(startX,endX),y2=Math.max(startY,endY);
                paint.reset(); paint.setStyle(Paint.Style.FILL); paint.setColor(0x44FFE500);
                canvas.drawRect(x1,y1,x2,y2,paint);
                paint.setStyle(Paint.Style.STROKE); paint.setStrokeWidth(2); paint.setColor(0xCCFFFFFF);
                canvas.drawRect(x1,y1,x2,y2,paint);
            }
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pageInfo.setTextColor(0xFFFFFFFF); pageInfo.setTextSize(12); pageInfo.setGravity(Gravity.CENTER);
        ...
        // Yer imi butonu
        Button btnBookmark=makeTopBtn("☆",0xFF0F3460);
        btnBookmark.setTextSize(18);

        // Gece modu butonu
        btnNight=makeTopBtn("🌙",0xFF0F3460);
        btnNight.setTextSize(16);
        ...
        topBar.addView(btnBookmark);
        topBar.addView(btnNight);
        ...
        for(int i=0;i<4;i++){
            Button cb=new Button(this); cb.setText(hlNames[i]);
            cb.setBackgroundColor(hlVals[i]);
            cb.setTextColor(i==0?0xFF000000:0xFFFFFFFF);
            cb.setTextSize(11); cb.setTypeface(null,android.graphics.Typeface.BOLD);
            ...
            cp.setMargins(3,0,3,0); cb.setLayoutParams(cp);
            colorBtns[i]=cb; colorRow.addView(cb);
            final String key=hlKeys[i]; final int idx=i;
            cb.setOnClickListener(v->{highlightColor=key;deleteMode=false;for(int j=0;j<4;j++)colorBtns[j].setAlpha(j==idx?1f:0.45f);});
        }
        colorBtns[0].setAlpha(1f); for(int i=1;i<4;i++)colorBtns[i].setAlpha(0.45f);
        ...
        btnUndo.setText("↩  GERİ AL"); btnUndo.setBackgroundColor(0xFF37474F);
        btnUndo.setTextColor(0xFFFFFFFF); btnUndo.setTextSize(13);
        btnUndo.setTypeface(null,android.graphics.Typeface.BOLD); btnUndo.setPadding(16,10,16,10);
        ...
        btnDelMode.setText("✕  İŞARETLEME SİL"); btnDelMode.setBackgroundColor(0xFF333333);
        btnDelMode.setTextColor(0xFFFFFFFF); btnDelMode.setTextSize(13);
        btnDelMode.setTypeface(null,android.graphics.Typeface.BOLD); btnDelMode.setPadding(16,10,16,10);
        LinearLayout.LayoutParams dmp=new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1);
        dmp.setMargins(8,0,0,0); btnDelMode.setLayoutParams(dmp);
        actionRow.addView(btnDelMode);
        highlightToolbar.addView(actionRow);

        // İçerik
        FrameLayout contentArea=new FrameLayout(this);
        contentArea.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,0,1));
        ...
        if(fileType.equals("TXT")){
            ...
            contentArea.addView(sv); topBar.setVisibility(View.GONE);
        } else {
            ...
            highlightOverlay.setLayoutParams(new FrameLayout.LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT,FrameLayout.LayoutParams.MATCH_PARENT));
        ...
        btnGo.setOnClickListener(v->{
            ...
            EditText input=new EditText(this); input.setInputType(InputType.TYPE_CLASS_NUMBER); input.setHint("1 - "+totalPages);
            b.setView(input);
            b.setPositiveButton("Git",(d,w)->{
                try{int p=Integer.parseInt(input.getText().toString())-1;if(p>=0&&p<totalPages)showPage(p);else Toast.makeText(this,"Gecersiz",Toast.LENGTH_SHORT).show();}
        ...
        btnBookmark.setOnClickListener(v->{
            AlertDialog.Builder b=new AlertDialog.Builder(this);
            b.setTitle("Yer İmi Ekle — Sayfa "+(currentPage+1));
            EditText input=new EditText(this); input.setHint("Başlık (isteğe bağlı)");
            b.setView(input);
            ...
            b.setNegativeButton("Yer İmlerini Gör",(d,w)->showBookmarksList());
            b.show();
        });

        // Gece modu
        btnNight.setOnClickListener(v->{
            if(!nightMode&&!sepiaMode){sepiaMode=true;btnNight.setText("🌅");Toast.makeText(this,"Sepia modu",Toast.LENGTH_SHORT).show();}
            else if(sepiaMode){sepiaMode=false;nightMode=true;btnNight.setText("☀️");Toast.makeText(this,"Gece modu",Toast.LENGTH_SHORT).show();}
            else{nightMode=false;btnNight.setText("🌙");Toast.makeText(this,"Normal mod",Toast.LENGTH_SHORT).show();}
        ...
        btnUndo.setOnClickListener(v->{
            ...
            else Toast.makeText(this,"Geri alınacak işaretleme yok",Toast.LENGTH_SHORT).show();
        });
    ...
    private Button makeTopBtn(String text,int color){
        Button btn=new Button(this); btn.setText(text); btn.setBackgroundColor(color);
        btn.setTextColor(0xFFFFFFFF); btn.setTextSize(14);
        btn.setTypeface(null,android.graphics.Typeface.BOLD); btn.setPadding(14,4,14,4); return btn;
    }

    private Button makeIconBtn(String text,int iconRes,int color){
        Button btn=new Button(this); btn.setText(text); btn.setBackgroundColor(color);
        btn.setTextColor(0xFFFFFFFF); btn.setTextSize(10);
        btn.setTypeface(null,android.graphics.Typeface.BOLD); btn.setGravity(Gravity.CENTER);
        try{Drawable icon=ContextCompat.getDrawable(this,iconRes);if(icon!=null){icon.setBounds(0,0,40,40);btn.setCompoundDrawables(null,icon,null,null);btn.setCompoundDrawablePadding(4);}}catch(Exception e){}
        LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1);
        lp.setMargins(3,0,3,0); btn.setLayoutParams(lp); btn.setPadding(4,8,4,8); return btn;
    }

    private void setupTouch(){
        highlightOverlay.setOnTouchListener((v,event)->{
            if(!highlightMode)return false;
            if(deleteMode){if(event.getAction()==MotionEvent.ACTION_DOWN){long id=highlightOverlay.removeTouched(event.getX(),event.getY());if(id>0){db.delete("page_highlights","id=?",new String[]{String.valueOf(id)});Toast.makeText(this,"Silindi",Toast.LENGTH_SHORT).show();}}return true;}
            switch(event.getAction()){
                case MotionEvent.ACTION_DOWN: highlightOverlay.startDraw(event.getX(),event.getY()); return true;
                case MotionEvent.ACTION_MOVE: highlightOverlay.updateDraw(event.getX(),event.getY()); return true;
                case MotionEvent.ACTION_UP:
                    float[]rect=highlightOverlay.finishDraw();
                    if(rect!=null){ContentValues cv=new ContentValues();cv.put("pdf_uri",pdfUri);cv.put("page",currentPage);cv.put("x1",rect[0]);cv.put("y1",rect[1]);cv.put("x2",rect[2]);cv.put("y2",rect[3]);cv.put("color",highlightColor);db.insert("page_highlights",null,cv);loadPageHighlights(currentPage);Toast.makeText(this,"İşaretlendi",Toast.LENGTH_SHORT).show();}
                    return true;
            }
            return false;
        });
        pageView.setOnTouchListener((v,event)->{
            ...
            switch(event.getActionMasked()){
                ...
                case MotionEvent.ACTION_MOVE:
                    if(touchMode==TOUCH_DRAG&&!scaleDetector.isInProgress()){float dx=event.getX()-lastTouchX,dy=event.getY()-lastTouchY;matrix.set(savedMatrix);matrix.postTranslate(dx,dy);clampMatrix();pageView.setImageMatrix(matrix);}
                    break;
    ...
    private void clampMatrix(){
        ...
        tX=Math.max(minTX,Math.min(tX,maxTX)); tY=Math.max(minTY,Math.min(tY,maxTY));
        values[Matrix.MTRANS_X]=tX; values[Matrix.MTRANS_Y]=tY; matrix.setValues(values);
    }

    private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener{
        @Override public boolean onScale(ScaleGestureDetector d){
            float[]v=new float[9];matrix.getValues(v);float cur=v[Matrix.MSCALE_X];
            float ns=Math.max(MIN_ZOOM,Math.min(cur*d.getScaleFactor(),MAX_ZOOM));
            matrix.postScale(ns/cur,ns/cur,d.getFocusX(),d.getFocusY());clampMatrix();pageView.setImageMatrix(matrix);return true;
        }
    }

    private void applyColorMode(Bitmap bmp) {
        if (nightMode) {
            // Gece: karanlık, göz dostu
    ...
    private void openTextFile(){
        try{InputStream is=getContentResolver().openInputStream(Uri.parse(pdfUri));BufferedReader r=new BufferedReader(new InputStreamReader(is));StringBuilder sb=new StringBuilder();String line;while((line=r.readLine())!=null)sb.append(line).append("\n");r.close();txtContent.setText(sb.toString());}
    ...
    private void showPage(int index){
        ...
        if(highlightOverlay!=null)highlightOverlay.loadData(new ArrayList<>(),new ArrayList<>());
        try{
            ...
            page.close(); pageView.setImageBitmap(bmp);
            applyColorMode(bmp);
            pageInfo.setText((index+1)+" / "+totalPages);
            loadPageHighlights(index);

            // Yer imi ikonunu güncelle
            Cursor bm=db.rawQuery("SELECT id FROM bookmarks WHERE pdf_uri=? AND page=?",new String[]{pdfUri,String.valueOf(index)});
            boolean hasBookmark=bm.moveToFirst(); bm.close();
            // btnBookmark referans yok burada — topbar'daki butona erişemiyoruz, OK
    ...
    private void loadPageHighlights(int page){
        if(highlightOverlay==null)return;
        List<long[]>ids=new ArrayList<>();List<float[]>rects=new ArrayList<>();
        Cursor c=db.rawQuery("SELECT id,x1,y1,x2,y2,color FROM page_highlights WHERE pdf_uri=? AND page=? ORDER BY id ASC",new String[]{pdfUri,String.valueOf(page)});
        while(c.moveToNext()){
        ...
        c.close(); highlightOverlay.loadData(ids,rects);
    }

    private void saveHighlight(String color,String label){
        AlertDialog.Builder b=new AlertDialog.Builder(this);
        b.setTitle(label+" — Sayfa "+(currentPage+1));
        LinearLayout layout=new LinearLayout(this); layout.setOrientation(LinearLayout.VERTICAL); layout.setPadding(32,16,32,8);
        EditText input=new EditText(this); input.setHint("Notunu yaz..."); input.setMinLines(2); layout.addView(input);
        TextView tl=new TextView(this); tl.setText("Etiket (isteğe bağlı):"); tl.setTextColor(0xFF888888); tl.setTextSize(12); tl.setPadding(0,16,0,8); layout.addView(tl);
        HorizontalScrollView hs=new HorizontalScrollView(this); LinearLayout tagRow=new LinearLayout(this); tagRow.setOrientation(LinearLayout.HORIZONTAL);
        final String[]selTag={""}; Button[]tagBtns=new Button[TAGS.size()];
        for(int i=0;i<TAGS.size();i++){String t=TAGS.get(i);Button tb=new Button(this);tb.setText(t);tb.setTextSize(10);tb.setTextColor(0xFFFFFFFF);tb.setBackgroundColor(0x446A1B9A);tb.setPadding(16,6,16,6);LinearLayout.LayoutParams tp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,ViewGroup.LayoutParams.WRAP_CONTENT);tp.setMargins(4,0,4,0);tb.setLayoutParams(tp);tagBtns[i]=tb;tagRow.addView(tb);final int idx=i;tb.setOnClickListener(cv->{selTag[0]=t;for(int j=0;j<tagBtns.length;j++)tagBtns[j].setBackgroundColor(j==idx?0xFF6A1B9A:0x446A1B9A);});}
        hs.addView(tagRow); layout.addView(hs); b.setView(layout);
        b.setPositiveButton("Kaydet",(d,w)->{ContentValues cv=new ContentValues();cv.put("pdf_uri",pdfUri);cv.put("page",currentPage);cv.put("color",color);cv.put("note",input.getText().toString());cv.put("tag",selTag[0]);db.insert("highlights",null,cv);Toast.makeText(this,"Kaydedildi",Toast.LENGTH_SHORT).show();});
